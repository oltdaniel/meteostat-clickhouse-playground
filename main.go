...
// == BEGIN config
const (
	// Amount of rows inserted per batch for station data
	INSERT_STATION_DATA_BATCH_SIZE = 250
	// After how many records should a log message be printed
...
func main() {
	...
	importDataCmd := &cli.Command{
		Name:  "data",
		Usage: "import station details",
		Action: func(cCtx *cli.Context) error {
			for _, stationId := range cCtx.Args().Slice() {
